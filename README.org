...
| =C-c C-s=     | input   | Queue steering message                 |
| =C-c C-q=     | input   | Queue follow-up message                |
...
| =@=           | input   | File reference (fuzzy search)          |
...
- *Message queuing*: Submit messages while the agent is working:
  - =C-c C-s= queues a /steering/ message, delivered after current tool execution
    (interrupts remaining tools)
  - =C-c C-q= queues a /follow-up/ message, delivered only after the agent finishes
